  ...
  return (
    <div className="avbrytPlanBekreftelse">
      ...
      <div className="knapperad">
        <Fareknapp
          onClick={() => {
            avbrytDialog(oppfolgingsplan.id);
          }}
        >
        ...
        >
          <div>
            {this.state.visBekreftelse && (
              <Lightbox lukkLightbox={this.lukkBekreftelse}>
                <AvbrytPlanBekreftelse oppfolgingsplan={oppfolgingsplan} avbrytDialog={avbrytDialog} />
              </Lightbox>
            )}

            {!godkjentPlan.tvungenGodkjenning && <p>{textBothApprovedOppfolgingsplan(arbeidstakerNavn)}</p>}
            {godkjentPlan.tvungenGodkjenning && (
              <TextForcedApprovedOppfolgingsplan rootUrl={rootUrl} text={tvungenGodkjenningText(arbeidstakerNavn)} />
            )}
            <GodkjennPlanTidspunkt gyldighetstidspunkt={oppfolgingsplan.godkjentPlan.gyldighetstidspunkt} />

            <GodkjentPlanDeltBekreftelse oppfolgingsplan={oppfolgingsplan} />

            <PlanEkspanderbar oppfolgingsplan={oppfolgingsplan} />
            {isGodkjentPlanDelKnapperAvailable(oppfolgingsplan) && (
