            ...
            {type === KANGJENNOMFOERES.TILRETTELEGGING && (
              <div className="arbeidsoppgave__tilrettelegging">
                {arbeidsoppgave.gjennomfoering.paaAnnetSted && (
                  <span>{texts.informasjonPanelArbeidsoppgaverEtterGjennomfoering.labels.sted}</span>
                )}
                {arbeidsoppgave.gjennomfoering.medMerTid && (
                  <span>{texts.informasjonPanelArbeidsoppgaverEtterGjennomfoering.labels.tid}</span>
                )}
                {arbeidsoppgave.gjennomfoering.medHjelp && (
                  <span>{texts.informasjonPanelArbeidsoppgaverEtterGjennomfoering.labels.hjelp}</span>
                )}
              </div>
            )}
            {type !== KANGJENNOMFOERES.KAN &&
              ...
              arbeidstaker.navn && (
                <div className="arbeidsoppgave__beskrivelse">
                  ...
                  {type === KANGJENNOMFOERES.TILRETTELEGGING && <q>{arbeidsoppgave.gjennomfoering.kanBeskrivelse}</q>}
                  {type === KANGJENNOMFOERES.KAN_IKKE && <q>{arbeidsoppgave.gjennomfoering.kanIkkeBeskrivelse}</q>}
                </div>
              )}
          </div>
        );
...
export const InformasjonPanelArbeidsoppgaver = ({ oppfolgingsdialog, arbeidsoppgaver, rootUrl }) => {
  ...
  const arbeidsoppgaverKanIkkeGjennomfoeres = arbeidsoppgaver.filter((arbeidsoppgave) => {
    return (
      arbeidsoppgave.gjennomfoering && arbeidsoppgave.gjennomfoering.kanGjennomfoeres === KANGJENNOMFOERES.KAN_IKKE
    );
  });
...
export const getEtikettType = (tiltak) => {
  switch (tiltak.status) {
    case STATUS_TILTAK.AVTALT:
      return 'suksess';
    case STATUS_TILTAK.IKKE_AKTUELT:
...
export const TiltakBeskrivelse = ({ tiltak }) => {
  return (
    <div className="godkjennPlanOversiktInfo__panel__tiltak--beskrivelse">
      <label key={`tiltak-besk-label-${tiltak.tiltakId}`}>{texts.tiltakBeskrivelse.label}</label>
  ...
  return (
    <div className="godkjennPlanOversiktInfo__panel__tiltak--beskrivelse">
      {tiltak.gjennomfoering && [
        <label key={`tiltak-gjenn-label-${tiltak.tiltakId}`}>{texts.tiltakOppfoelging.label}</label>,
  ...
  return (
    <div className="godkjennPlanOversiktInfo__panel__tiltak--beskrivelse">
      {tiltak.beskrivelseIkkeAktuelt && [
        <label key={`tiltak-gjenn-label-${tiltak.tiltakId}`}>{texts.tiltakBeskrivelseIkkeAktuelt.label}</label>,
...
export const TiltakForeslaattAv = ({ tiltak }) => {
  return (
    <div className="godkjennPlanOversiktInfo__panel__tiltak--foreslaattAv">
      {tiltak.opprettetAv && [
        <label key={`tiltak-gjenn-label-${tiltak.tiltakId}`}>{texts.tiltakForeslaattAv.label}</label>,
      ...
      {tiltakListe.map((tiltak, idx) => {
        return (
          <div className="godkjennPlanOversiktInfo__panel__tiltak" key={idx}>
            ...
            {tiltak.status && (
              <EtikettBase mini type={getEtikettType(tiltak)}>
                {getTiltakStatus(tiltak)}
              </EtikettBase>
            )}
            <TiltakBeskrivelse tiltak={tiltak} />
            <TiltakForeslaattAv tiltak={tiltak} />
            {tiltak.status === STATUS_TILTAK.AVTALT && <TiltakOppfoelging tiltak={tiltak} />}
            {tiltak.status === STATUS_TILTAK.IKKE_AKTUELT && <TiltakBeskrivelseIkkeAktuelt tiltak={tiltak} />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="godkjennPlanOversiktInformasjon">
      ...
      {oppfolgingsdialog.tiltakListe.length > 0 && (
        <InformasjonPanelTiltak tiltakListe={sorterTiltakerEtterStatus(oppfolgingsdialog.tiltakListe)} />
      )}
    </div>
  );
};
